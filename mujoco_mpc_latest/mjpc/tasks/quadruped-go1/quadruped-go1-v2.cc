...
#include "mjpc/tasks/quadruped-go1/quadruped-go1-v2.h"
...
namespace mjpc {
std::string QuadrupedGo1Hill::XmlPath() const {
  return GetModelPath("quadruped-go1/task_hill2.xml");
}
std::string QuadrupedGo1Flat::XmlPath() const {
  return GetModelPath("quadruped-go1/task_flat1.xml");
}
std::string QuadrupedGo1Hill::Name() const { return "Quadruped Go1 Hill"; }
std::string QuadrupedGo1Flat::Name() const { return "Quadruped Go1 Flat"; }


void QuadrupedGo1Flat::Residual(const mjModel* model, const mjData* data,
...
//  ============  transition  ============
void QuadrupedGo1Flat::Transition(const mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
  ...
  // save mode
  current_mode_ = static_cast<Go1Mode>(mode);
...
void QuadrupedGo1Flat::ModifyScene(const mjModel* model, const mjData* data,
  ...
  for (Go1Foot foot : kFootAll) {  // set to foot horizontal positions
...
void QuadrupedGo1Flat::Reset(const mjModel* model) {
...
void QuadrupedGo1Flat::AverageFootPos(double avg_foot_pos[3],
...
double QuadrupedGo1Flat::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void QuadrupedGo1Flat::Walk(double pos[2], double time) const {
...
// get gait
QuadrupedGo1Flat::Go1Gait QuadrupedGo1Flat::GetGait() const {
  if (current_mode_ == kModeBiped)
    return kGaitTrot;
  return static_cast<Go1Gait>(ReinterpretAsInt(parameters[gait_param_id_]));
}

// return normalized target step height
double QuadrupedGo1Flat::StepHeight(double time, double footphase,
...
void QuadrupedGo1Flat::FootStep(double step[kNumFoot], double time,
                             Go1Gait gait) const {
...
// height during flip
double QuadrupedGo1Flat::FlipHeight(double time) const {
...
void QuadrupedGo1Flat::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2*mjPI;
...
// -----------------------------------------------------------------------
void QuadrupedGo1Hill::Residual(const mjModel* model, const mjData* data,
                             double* residual) const {
  // ---------- Residual (0) ----------
...
// -----------------------------------------------
void QuadrupedGo1Hill::Transition(const mjModel* model, mjData* data) {
