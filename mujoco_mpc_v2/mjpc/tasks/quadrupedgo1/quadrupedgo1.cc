...
namespace mjpc {
std::string QuadrupedHillgo1::XmlPath() const {
  return GetModelPath("quadruped/task_hill.xml");
}
std::string QuadrupedFlatgo1::XmlPath() const {
  return GetModelPath("quadruped/task_flat.xml");
}
std::string QuadrupedHillgo1::Name() const { return "Quadruped Hill Go1"; }
std::string QuadrupedFlatgo1::Name() const { return "Quadruped Flat Go1"; }


void QuadrupedFlatgo1::Residual(const mjModel* model, const mjData* data,
...
//  ============  transition  ============
void QuadrupedFlatgo1::Transition(const mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
...
void QuadrupedFlatgo1::ModifyScene(const mjModel* model, const mjData* data,
...
void QuadrupedFlatgo1::Reset(const mjModel* model) {
...
void QuadrupedFlatgo1::AverageFootPos(double avg_foot_pos[3],
...
double QuadrupedFlatgo1::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void QuadrupedFlatgo1::Walk(double pos[2], double time) const {
...
// get gait
QuadrupedFlatgo1::A1Gait QuadrupedFlatgo1::GetGait() const {
  if (current_mode_ == kModeBiped)
    return kGaitTrot;
...
double QuadrupedFlatgo1::StepHeight(double time, double footphase,
...
void QuadrupedFlatgo1::FootStep(double step[kNumFoot], double time,
                             A1Gait gait) const {
...
// height during flip
double QuadrupedFlatgo1::FlipHeight(double time) const {
...
void QuadrupedFlatgo1::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2*mjPI;
...
// -----------------------------------------------------------------------
void QuadrupedHillgo1::Residual(const mjModel* model, const mjData* data,
                             double* residual) const {
  // ---------- Residual (0) ----------
...
// -----------------------------------------------
void QuadrupedHillgo1::Transition(const mjModel* model, mjData* data) {
